...
| 3.11 | Backups: ✅ **documents** — versioned with a one-day noncurrent tail, continuously replicated into a write-locked `backup` bucket under its own key with 35-day retention (`infra/modules/backup`), and **proved on 18 September 2026** by restoring a genuinely deleted document from it: 404 in the live bucket, intact in the backup, size and ETag matching. ❌ **database — there is no backup at all.** The claim that "Supabase's daily backups cover the database" was false: the org is on the Free plan, which gets none, and nothing self-managed replaced them. The drill record and the operator's two options are at the top of `docs/runbooks/restore.md`. 2095 stays open — its criterion is the application running against restored data, and half the data cannot be restored | 2094, 2095 |
