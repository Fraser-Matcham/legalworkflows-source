...
| 3.10 | Signed-URL round trip against real S3: ✅ the code half — `backend/src/lib/storageRegion.ts` resolves the signing region from `R2_REGION` (`auto` for R2, the bucket's own region for real S3), covered by `storageRegion.test.ts`; a presigned URL signed for the wrong region is rejected by S3 and reads like a permissions error. ⏳ The live check needs the deployed stack — the procedure is in [`docs/runbooks/first-apply.md`](../../runbooks/first-apply.md), and Stage 4 Task 5 steps 4 and 7 walk the same path through the product | 2044 |
...
| 4.4 | Rollback: ✅ `.github/workflows/rollback.yml` (revision numbers from the previous release's summary). 🟡 Drilled against production 18 September 2026 and the ECS circuit breaker **did not** roll a broken deploy back — an unpullable image stalls the deployment rather than failing it, and it was restored by hand after 17 minutes. Availability held throughout (33/33 probes 200). The deploy workflow's `wait services-stable` plus revision assertion is what actually catches it. The runtime-failure case is untested | 2052 |
...
| 4.14 | 🟡 Post-launch monitoring window — 16 CloudWatch alarms are armed, all with actions enabled, covering 5xx, latency, CPU, memory, unhealthy targets, running-task count and the readiness probe. Blocked: both SNS alert topics have **zero subscriptions**, so every alarm fires into a topic with no listeners. Needs an address subscribed and a human to confirm it | 2107 |
